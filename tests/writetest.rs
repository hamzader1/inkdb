fn dump_tree(pager: &mut inkdb::pager::pager::Pager<inkdb::vfs::disk::DiskVfs>, root: u32) -> Vec<(u32, Vec<u8>)> {
    let mut out = Vec::new();
    let mut stack = vec![root];
    let mut seen = std::collections::HashSet::new();
    while let Some(p) = stack.pop() {
        if !seen.insert(p) || seen.len() > 40 { continue; }
        let Ok(mut g) = pager.get(p) else { continue; };
        let bytes = g.bytes_as_ref().to_vec();
        let t = bytes[if p == 1 { 100 } else { 0 }];
        out.push((p, bytes));
        if t == 2 || t == 5 {
            let n = u16::from_be_bytes([out.last().unwrap().1[if p == 1 { 103 } else { 3 }], out.last().unwrap().1[if p == 1 { 104 } else { 4 }]]) as usize;
            ...
            for i in 0..n.min(200) {
                ...
                if ptr + 4 <= b.len() {
                    ...
                    if child != 0 && child < 100000 { stack.push(child); }
                }
            }
            let b = &out.last().unwrap().1;
...
fn btree_index_split_roundtrip() {
    ...
    let root = pager.allocate_new_page().unwrap();
    {
        ...
        BTreePage::new_from_raw_bytes(root, BTreePageType::LeafIndex, g.bytes_as_mut_unchecked(), ps, ps).unwrap();
    }
    for i in 0..2000u64 {
        let snap_before = if i == 519 { Some(dump_tree(&mut pager, root)) } else { None };
        ...
        if let Err(e) = BTree::new(root, &mut pager).insert(&key, &mut bytes) {
            ...
            if let Some(before) = snap_before {
                ...
                for (b, a) in before.iter().zip(after.iter()) {
                    if b.1 != a.1 {
                        eprintln!("DIFF page={} before={:x?} after={:x?}", b.0, &b.1[..16], &a.1[..16]);
                    }
                }
            }
            panic!("row {}", i);
...
{
    let mut g = pager.get(root).unwrap();
    let pg = inkdb::storage::page::BTreePage::new(root, 4096, 4096, g.bytes_as_ref()).unwrap();
    if pg.page_type().unwrap() as u8 == 2 || pg.page_type().is_err() {
        ...
        let mut h: u64 = 0;
        for b in g.bytes_as_ref()[..16].iter() { h = h.wrapping_shl(8) | (*b as u64); }
        let prev = LASTH.swap(h, std::sync::atomic::Ordering::SeqCst);
        if h != prev && prev != 0 {
            eprintln!("HDR-CHANGE row={} {:x?} (rmp={:?})", i, &g.bytes_as_ref()[..16], r);
        }
        let r = r.unwrap();
...
if let Err(e) = BTree::new(root, &mut pager).insert(&key, &mut bytes) {
    ...
    let mut seen = std::collections::HashSet::new();
    while let Some(p) = stack.pop() {
        if !seen.insert(p) || seen.len() > 30 { continue; }
        let mut g = match pager.get_mut(p) {
            Ok(g) => g,
            Err(e) => { eprintln!("  page={} GET ERR {:?}", p, e); continue; }
        };
        let pg = match inkdb::storage::page::BTreePage::new(p, 4096, 4096, g.bytes_as_mut_unchecked()) {
            Ok(pg) => pg,
            Err(e) => { eprintln!("  page={} PARSE ERR {:?}", p, e); continue; }
        };
        ...
        for c in 0..n {
            match pg.cell(c) {
                Ok(_) => {
                    ...
                    eprintln!("  page={} t={} cell{} len={} bytes={:x?}", p, t, c, bytes.len(), &bytes[..bytes.len().min(10)]);
                }
        ...
        if t == 2 || t == 5 {
            ...
            if let Ok(Some(r)) = pg.right_most_ptr() { stack.push(r); }
        }
    }
    panic!("row {}: {:?}", i, e);
}
